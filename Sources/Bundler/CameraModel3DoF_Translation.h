...
namespace Bundler { namespace CameraModels {
	...
	class CameraModel3DoF_Translation : public Camera
	{
	public:
	
		static constexpr const uint cameraParameterCount = 3;
	...
	public:
		...
		CameraModel3DoF_Translation( ) __CPU_ONLY
		{
		}
	
		CameraModel3DoF_Translation( __in const Camera* pCamera ) __CPU_ONLY
		{
			Initialize( pCamera );
		}
		...
		CameraModel3DoF_Translation& operator=( __in const CameraModel3DoF_Translation& src ) __CPU_ONLY = default;
	
		void SetCopy( __in const CameraModel3DoF_Translation* pSource ) __CPU_ONLY
		{
			*this = *pSource;
		}
