...
namespace Bundler { namespace CameraModels {
	...
	class CameraModel6DoF : public Camera
	{
	public:
	...
	public:
		...
		CameraModel6DoF( ) __CPU_ONLY
		{
			m_currentRotationCount = 0;
		}

		CameraModel6DoF( __in const Camera* pCamera ) __CPU_ONLY
		{
			Initialize( pCamera );
		}
		...
		CameraModel6DoF< maxRotations >& operator=( __in const CameraModel6DoF< maxRotations >& src ) __CPU_ONLY = default;

		void SetCopy( __in const CameraModel6DoF< maxRotations >* pSource ) __CPU_ONLY
		{
			*this = *pSource;
		}
